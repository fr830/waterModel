app.factory('userManageService', function ($parse, Restangular, appService, modelService, enumService) {
    ...
    return {
        ...
        add: function (model) {
            return res.all('AddUser').post(model);//post请求穿多个参数需要用all来配置路由
        },
        update: function (model) {
            return res.all('UpdateUser').customPUT(model);
        },
        reset:function(id){
        ...
        freeze:function(id){
            return res.one('DisableUser').get({ 'UserId': id });
        },
