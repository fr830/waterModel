...
/* 1 */
	(function(module, exports, __webpack_require__) {
		...
		var dragStore = (_dragStore = {
				...
				// 监听拖动
				onDragOver: function onDragOver(pageX, pageY, clientX, clientY) {
					// console.log(pageX, pageY, clientX, clientY);
					this.mousePosition = [pageX, pageY];
					// this.mousePosition = [clientX, clientY]; // 2018.10.1
					// this.targetIndex = this.collision(pageX, pageY);
					this.targetIndex = this.collision(clientX, clientY); // 2018.10.1
					// this.setIconPosition(pageX, pageY);
					this.setIconPosition(clientX, clientY);		// solve the icon position problem
					if (this.targetIndex >= 0) {
						// console.log('on onDragOver');
						// 判断是否在目标外 是的话表示刚刚进入
						if (!this._inTarget) {
						...
						var params = {
							...
							pageX: pageX,
							pageY: pageY,
							clientX: clientX,		// 2018.10.1
							clientY: clientY,		// 2018.10.1
							methods: methods
						};
			...
			_defineProperty(_dragStore, _config.REMOVE_ANIMATION_TYPES.fade,
				function() {
			...
			_defineProperty(_dragStore, _config.REMOVE_ANIMATION_TYPES.blost,
				function() {
					var time = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 150;
			...
			_defineProperty(_dragStore, _config.REMOVE_ANIMATION_TYPES.back,
				function() {
			...
			_defineProperty(_dragStore, 'removeMark',
				function removeMark() {
					var _this5 = this;
			...
			_defineProperty(_dragStore, 'destroyDrop',
				function destroyDrop(name) {
					var _this6 = this;
			...
			_defineProperty(_dragStore, 'removeDrop',
			...
			_defineProperty(_dragStore, 'getStateIconNode',
				function getStateIconNode() {
		...
		var Drag = function () {
			...
			_createClass(Drag, [{
				...
				value: function addEventListenerPC() {
					...
					function onMouseDown(e) {
						...
						var clientX = e.clientX,
							clientY = e.clientY;	// 2018.10.1

						// this.mouseDownPosition = { left: pageX, top: pageY };
						this.mouseDownPosition = { left: clientX, top: clientY }; // 2018.10.1
					...
					dom.addEventListener('mousedown', onMouseDown.bind(this));
				}
			}, {
				...
				value: function onElMousemove(e) {
					...
					var EMIT_LENGTH = 3;
					if (Math.abs(pageX - left) < EMIT_LENGTH && Math.abs(pageY - top) < EMIT_LENGTH) return;	// 移动3个位置，才算移动开始
				...
				value: function onMarkMouseMove(e) {
					// console.log(e);
					if (!_store2.default.draggedNode) {
						// console.log('not default dragnode', e.touches);
						return;
					}
					...
					var clientX = e.clientX,	// 2018.10.1
					clientY = e.clientY;

					// var translateX = pageX - this.mouseDownPosition.left;
					// var translateY = pageY - this.mouseDownPosition.top;
					var translateX = clientX - this.mouseDownPosition.left;		// solve the clone node position problem
					var translateY = clientY - this.mouseDownPosition.top;
					//console.log(this.mouseDownPosition);
					_store2.default.draggedNode.style.transform = 'translate(' + translateX + 'px,' + translateY + 'px)';
					_store2.default.onDragOver(pageX, pageY, clientX, clientY); //2018.10.1
				...
				value: function checkOptions(options) {
					...
					for (var option in baseOptions) { ! options[option] && (options[option] = baseOptions[option]);
					}
					return options;
				}
			}, {
				...
				value: function setMarkStyle() {
					var markStyle = {
						position: 'fixed',		// 2018.10.11, solve the dragging after scrolling the page
						top: '0',
						left: '0',
						width: '100%',
						height: '100%',
				...
				value: function setIconStyle() {
					...
					style.display = 'none';
					style.position = 'fixed';		// 2018.10.11
					style.width = '20px';
					style.height = '20px';
					style.zIndex = '10001';
				}
		...
		var Drop = function () {
			...
			_createClass(Drop, [{
				...
				value: function checkOptions(options) {
					...
					for (var option in baseOptions) { ! options[options] && (options[options] = baseOptions[option]);
					}
					return options;
				}
			}, {
				...
				value: function setStorePositions() {
					var _el$getBoundingClient = this.el.getBoundingClientRect(),	// 2018.10.11, useing clienX and clientY
