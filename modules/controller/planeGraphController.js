app.controller('planeGraphController', function ($scope, $rootScope,Restangular,$state,getPointDataDetailService, $stateParams,$timeout,showDataService, NgTableParams, userProjectService, $filter, mapFocusService) {
...
$scope.cutPlanGraph = function(id){
    ...
    if(!$scope.model.Depth){
        swal("请填写您所需要展示的水深");
        return;
    }else if($scope.model.Depth>$scope.maxDepth){
        swal('当前文件测得的最大水深为'+$scope.maxDepth+','+'请重新选择输入水深');
        return;
    }
    // console.log("当前地图中心点：" + map.getCenter().lng + "," + map.getCenter().lat);
    var bounds = mapAll.getBounds();
    var sw = bounds.getSouthWest();       //获取西南角的经纬度(左下端点)
    var ne = bounds.getNorthEast();       //获取东北角的经纬度(右上端点)
    var str_sw_ne = sw.lng+','+ ne.lat+','+ne.lng+','+sw.lat;
    // console.log(sw.lng, ne.lat);           //获取西北角的经纬度(左上端点)
    // console.log(ne.lng, sw.lat);          //获取东南角的经纬度(右下端点)
    // console.log(map.getZoom()) ; //当前缩放比
    ...
    $scope.height = document.getElementById("main1").offsetHeight;
    $scope.width = document.getElementById("main1").offsetWidth;
    var model={
        'Zoom':mapAll.getZoom(),
        'Lng':mapAll.getCenter().lng,
        'Lat':mapAll.getCenter().lat,//中心点坐标
        'PointDataIdStr':str_node,//点id
        'ParamDictionaryId':$scope.model.ParamDictionaryId,//参数id
        'vertex': str_sw_ne, //左上右下经纬度
        'width':$scope.width,
        'height':$scope.height,
        'targetDepth':$scope.model.Depth,
    }
    console.log(model);
    $('.page-loader-wrapper').show();
    // var src= Restangular.configuration.baseUrl+'/Map/Snap?'
    //     +'Lat='+mapAll.getCenter().lat
    //     +'&Lng='+mapAll.getCenter().lng
    //     +'&ParamDictionaryId='+$scope.model.ParamDictionaryId
    //     +'&PointDataIdStr='+str_node
    //     +'&Zoom='+mapAll.getZoom()
    //     +'&height='+$scope.height
    //     +'&targetDepth='+$scope.model.Depth
    //     +'&vertex='+str_sw_ne
    ...
    getPointDataDetailService.showCutGraph(model).then(function (data) {
        console.log(data);
        var str_regular = Restangular.configuration.baseUrl;
        var regulalar = str_regular.substring(0,str_regular.length-4);
       
        $scope.model.imgSnapSrc=regulalar+data.result.snapImg;
 ...
 //                }, 50);
 //            }
 //    imgLoad(document.getElementById('img'), function() {
 //        $('.page-loader-wrapper').fadeOut();
 //    })
